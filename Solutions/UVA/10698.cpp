...
int main(){
    ...
    while(cin >> T >> g && (T != 0 || g != 0)){
        ...
        REP(i, 0, T)
            cin >> s, a[s].name = s;
        string s1, s2;
        int res1, res2; char shit;
        while(g--){
            cin >> s1 >> res1 >> shit >> res2 >> s2;
        ...
        REP(i, 0, T){
            cout << setw(2) << i+1 << "." << setw(16) << vec[i].name << setw(4) << vec[i].total_points_earned << setw(4) << vec[i].games_played
                 << setw(4) << vec[i].goals_scored << setw(4) << vec[i].goals_against << setw(4)
                 << vec[i].goals_scored-vec[i].goals_against;
                 if(vec[i].games_played)
                    cout << setw(7) << fixed << setprecision(2) << vec[i].total_points_earned/double(vec[i].games_played*3) * 100<< endl;
                 else
                    cout << setw(7) << "N/A" << endl;
            ++i;
            while(i < T && vec[i] == vec[i-1]){
                cout << setw(19) << vec[i].name << setw(4) << vec[i].total_points_earned << setw(4) << vec[i].games_played
                     << setw(4) << vec[i].goals_scored << setw(4) << vec[i].goals_against << setw(4)
                     << vec[i].goals_scored-vec[i].goals_against;
                 if(vec[i].games_played)
                    cout << setw(7) << fixed << setprecision(2) << vec[i].total_points_earned/double(vec[i].games_played*3) * 100<< endl;
