...
struct PT{
	...
	friend double dist(const PT&a, const PT&b){
...
PT arr[16];
double memo[16][1<<16];
double solve(int idx = 0, MASK mask = MASK()){
    if(idx == 2*n)
        return 0;
    double &ret = memo[idx][mask.to_ulong()];
    if(!isnan(ret))
        return ret;
    if(mask[idx])
        return ret = solve(idx+1, mask);
    ret = 1e9;
    for(int i = 0; i < 2*n; ++i){
        if(!mask[i] && i != idx){
            MASK tmp = mask;
            tmp[i] = tmp[idx] = 1;
            ret = min(ret, dist(arr[idx], arr[i])+solve(idx+1, tmp));
        }
    }
    return ret;
}
